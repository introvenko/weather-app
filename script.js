...
let lon = '';
const container = document.querySelector('.container');
container.style.display = 'none'; // Приховуємо контейнер за замовчуванням
const currentWeather = document.createElement('section');
currentWeather.className = 'current-weather';
...
async function loadWeather(lat, lon, city, country) {
    ...
    try {
        ...
        if (data) {
            getWeather(data, city, country);
            getFutureWeather(data);
            container.style.display = 'block'; // Відображаємо container після завантаження даних
        } else {
            container.innerHTML = "Error fetching data";
        }
    } catch (error) {
...
function applyTheme(isDarkTheme) {
    if (isDarkTheme) {
        ...
        lightModeImages.forEach(image => image.setAttribute('hidden', true));
        darkModeImages.forEach(image => image.removeAttribute('hidden'));
    } else {
        body.classList.remove('dark-theme');
        switcher.textContent = 'Dark Theme';

        darkModeImages.forEach(image => image.setAttribute('hidden', true));
        lightModeImages.forEach(image => image.removeAttribute('hidden'));
    }
}
